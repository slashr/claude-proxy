...
4. The worker uses Claude's `bypassPermissions` mode in both read and write modes so headless jobs and PR workflows do not stall on approval prompts. Claude's auto-approval is the only worker-side permission control; the user must explicitly authorize the requested scope before routing it. The separate direct-Codex shell guard remains active so Codex does not silently substitute itself for Claude.

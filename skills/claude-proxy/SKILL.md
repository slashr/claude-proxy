...
1. The prompt hook provides a Claude worker job ID. Immediately call `show_claude_worker`, then call `wait_for_claude_worker` for the same job ID. Related prompts resume the same Claude session automatically.
...
4. The prompt hook uses plan mode for read-only work. When the user explicitly asks to implement, edit, create, install, configure, or otherwise change files, it uses Claude's `acceptEdits` mode within that requested scope. This is not permission bypass and does not authorize external-state changes.

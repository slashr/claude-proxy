...
1. The prompt hook provides a Claude worker job ID. Call `claude_is_working` for that job ID and use its two-minute default wait; never request a poll shorter than one minute. If it returns `timed_out: true` but `state` is still `running`, call it again—do not write a final answer, call the worker stuck, or ask the user to reattach files. Every detached launch starts a fresh Claude conversation to prevent a stale, unrelated session from blocking startup or leaking across tasks. Follow-ups receive a bounded, workspace-scoped handoff packet with the prior user request, worker result, workspace, and Git state; it never includes private Codex reasoning or raw tool transcripts. Workers have no time limit, so keep polling for as long as the job stays running, however many polls that takes.

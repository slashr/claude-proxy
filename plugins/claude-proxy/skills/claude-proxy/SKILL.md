...
1. The prompt hook provides a Claude worker job ID. Call `claude_is_working` for that job ID and use its two-minute default wait; never request a poll shorter than one minute. If it returns `timed_out: true`, call it again. Follow-ups in the same Codex task and workspace resume the matching Claude session, including short replies such as `pr?`; separate Codex tasks remain isolated. Say `fresh Claude` or `new Claude task` to intentionally start over. Fresh or recovered sessions receive a bounded handoff packet with the prior user request, worker result, workspace, and Git state; it never includes private Codex reasoning or raw tool transcripts. Workers have no time limit, so keep polling for as long as the job stays running, however many polls that takes.
